...
Project goals:
•	A central system 
A system which provides a suitable system for types of user and all user levels to be able to input, use tools and upload onto a centralised system. 
•	A centralised storage area 
A server or cloud storage to store all data where all user are able collect or upload data they are permitted too. 
•	A scalable system 
A System that can be scaled apon in 5 to 10 year period to account for further growth of the business
•	Improved data process 
Its important to comply with the GDPR so implementing data governance policies to data management to over time improve data quality, Also keeping documents accurate and up to date to prevent incorrect insights. 
